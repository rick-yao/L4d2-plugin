...
public void OnPluginStart()
{
	...
	ChanceAverageHealth		  = CreateConVar("l4d2_tank_draw_average_health_chance", "10", "平均生命值的概率(平均所有站立状态幸存者的实际血量，清空临时血量) / Probability of average health", FCVAR_NONE);

	ChanceClearAllSurvivorHealth	  = CreateConVar("l4d2_tank_draw_clear_all_survivor_health_chance", "10", "清空所有人血量概率(将所有站立状态幸存者实际血量变成1，清空临时血量) / Probability of clearing all survivors' health", FCVAR_NONE);

	L4D2TankDrawDebugMode		  = CreateConVar("l4d2_tank_draw_debug_mode", "0", "是否开启调试模式，修改后tank被任意武器击杀即可抽奖，使用!x开启击杀tank菜单 / Enable debug mode, draw after tank is killed when enabled, use !x open kill tank menu", false, false);

	ChanceDecreaseHealth		  = CreateConVar("l4d2_tank_draw_decrease_health_chance", "10", "抽奖受到伤害的概率(伤害超过血量时，将血量设置为1而不是倒地) / Probability of receiving damage", FCVAR_NONE);
	...
	ChanceDisarmAllSurvivor		  = CreateConVar("l4d2_tank_draw_disarm_all_survivor_chance", "10", "所有人缴械概率(清空所有槽位道具) / Probability of disarming all survivors", FCVAR_NONE);
	...
	ChanceDisableGlow		  = CreateConVar("l4d2_tank_draw_disable_glow_chance", "30", "取消人物光圈概率(取消人物轮廓光圈，人物头上的名字依然保留，不建议使用，很多插件会修改光圈) / Probability of disabling survivor glow", FCVAR_NONE);

	ChanceIncreaseGravity		  = CreateConVar("l4d2_tank_draw_gravity_increased_chance", "10", "增加单人重力的概率(爬梯子可以解除buff) / Probability of increasing single player gravity", FCVAR_NONE);
	...
	ChanceMoonGravityOneLimitedTime	  = CreateConVar("l4d2_tank_draw_gravity_moon_single_chance", "10", "抽奖者单人获得限时月球重力的概率(爬梯子可以解除buff) / Probability of single player getting limited time moon gravity", FCVAR_NONE);
	...
	WorldMoonGravity		  = CreateConVar("l4d2_tank_draw_gravity_moon_world", "80", "月球重力时世界重力参数，世界重力正常值为800(游戏全局参数，爬梯子不可解除buf，非限时重复抽取会恢复默认值) / World gravity parameter for moon gravity, normal world gravity is 800", false, false);
	ChanceLimitedTimeWorldMoonGravity = CreateConVar("l4d2_tank_draw_gravity_moon_world_chance", "10", "获得限时世界重力改为月球重力的概率(重复抽取会重置计时器) / Probability of getting limited time world moon gravity", FCVAR_NONE);
	...
	ChanceIncreaseHealth		  = CreateConVar("l4d2_tank_draw_health_increase_chance", "10", "增加生命值的概率(幸存者总血量无法突破服务器设置的最大生命值) / Probability of increasing health", FCVAR_NONE);
	...
	ChanceInfiniteAmmo		  = CreateConVar("l4d2_tank_draw_infinite_ammo_chance", "10", "无限弹药的概率(游戏全局参数，重复抽取会恢复默认，和主武器无限弹药只能同时有一个生效) / Probability of infinite ammo", FCVAR_NONE);
	...
	ChanceInfinitePrimaryAmmo	  = CreateConVar("l4d2_tank_draw_infinite_primary_ammo_chance", "10", "主武器无限弹药的概率(游戏全局参数，重复抽取会恢复默认，和无限弹药只能同时有一个生效) / Probability of infinite primary ammo", FCVAR_NONE);

	ChanceInfiniteMelee		  = CreateConVar("l4d2_tank_draw_infinite_melee_chance", "5", "无限近战范围的概率(游戏全局参数，重复抽取会恢复默认) / Probability of infinite melee range", FCVAR_NONE);
	...
	ChanceResetAllSurvivorHealth	  = CreateConVar("l4d2_tank_draw_reset_all_survivor_health_chance", "30", "重置所有人血量概率(将所有人血量设置为100，清空倒地次数记数) / Probability of resetting all survivors' health", FCVAR_NONE);
	...
	ChanceTimerBomb			  = CreateConVar("l4d2_tank_draw_timer_bomb_chance", "30", "变成定时炸弹概率(重复抽取会取消计时) / Probability of becoming a timer bomb", FCVAR_NONE);
	...
	ChanceFreezeBomb		  = CreateConVar("l4d2_tank_draw_freeze_bomb_chance", "30", "变成冰冻炸弹概率(重复抽取会取消计时) / Probability of becoming a freeze bomb", FCVAR_NONE);
